...
static void	rotate_around_center(t_triangle *triangle, t_smat_3d m)
{
	...
	v[0] = coord_3d_minus(triangle->p1, triangle->centroid);
	v[1] = coord_3d_minus(triangle->p2, triangle->centroid);
	v[2] = coord_3d_minus(triangle->p3, triangle->centroid);
	v[3] = coord_3d_minus(triangle->c, triangle->centroid);
	i = -1;
	while (++i < 4)
		v[i] = smat_vec_3d_product(m, v[i]);
	triangle->p1 = coord_3d_plus(triangle->centroid, v[0]);
	triangle->p2 = coord_3d_plus(triangle->centroid, v[1]);
	triangle->p3 = coord_3d_plus(triangle->centroid, v[2]);
	triangle->c = coord_3d_plus(triangle->centroid, v[3]);
	triangle->basis.u = coord_3d_minus(triangle->p2, triangle->p1);
	v[0] = coord_3d_minus(triangle->p3, triangle->p1);
