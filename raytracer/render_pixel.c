...
t_coord_3d	get_first_grid_center(t_coord_3d c, t_camera *camera)
{
	...
	v1 = coord_3d_plus(camera->pixel.v, camera->sub_pixel.u);
	v2 = coord_3d_plus(camera->pixel.u, camera->sub_pixel.v);
	v1 = coord_3d_minus(v1, v2);
	return (coord_3d_plus(c, scalar_product(v1, 0.5)));
}
			...
			int nb)
{
	...
	while (++i < nb)
	{
		...
		while (j < nb)
		{
			...
			j += inc;
			p[1] = coord_3d_plus(p[0], scalar_product(camera->sub_pixel.u, j));
		}
		p[0] = coord_3d_minus(p[0], camera->sub_pixel.v);
	}
	return (rgb);
}
...
t_rgb		super_sample(void **entities, t_camera *camera, t_ray ray, int nb)
{
	...
	while (++i < nb - 1)
	{
		...
		while (++j < nb - 1)
		{
			ray.direction = coord_3d_minus(p[1], ray.origin);
			rgb = rgb_sum(rgb, get_color(ray, entities));
			p[1] = coord_3d_plus(p[1], camera->sub_pixel.u);
		}
		p[0] = coord_3d_minus(p[0], camera->sub_pixel.v);
	}
	return (rgb);
