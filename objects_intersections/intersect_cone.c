...
static void	check_ref_cone(double *t, t_cone *cone, t_coord_3d p, t_coord_3d d)
{
	...
	q1 = coord_3d_plus(p, scalar_product(d, t[0]));
	a = dot_product(cone->basis.w, coord_3d_minus(q1, cone->vertex));
	q2 = coord_3d_plus(p, scalar_product(d, t[1]));
	b = dot_product(cone->basis.w, coord_3d_minus(q2, cone->vertex));
...
double		intersect_cone(t_ray ray, t_cone *cone)
{
	...
	v = coord_3d_minus(ray.origin, cone->vertex);
