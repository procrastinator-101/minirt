...
void	get_triangle_texture_center(t_triangle *triangle)
{
	...
	v1 = coord_3d_sub(triangle->p2, triangle->p1);
	v2 = coord_3d_sub(triangle->p3, triangle->p1);
	len_v1 = coord_3d_len(v1);
	triangle->centroid = coord_3d_add(triangle->p1, scalar_product(v1, 0.5));
	v = scalar_product(triangle->basis.u, triangle->texture.width / 2.0);
	if (triangle->texture.width == len_v1 || dot_product(v1, v2) >= 0)
		triangle->c = coord_3d_add(triangle->p1, v);
	else
		triangle->c = coord_3d_sub(triangle->p2, v);
	triangle->c = coord_3d_add(triangle->c, \
