...
/*   Updated: 2020/12/24 11:30:59 by yarroubi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_coord_3d	triangle_normal(t_triangle *triangle, t_coord_3d p, t_coord_3d d)
{
	...
	if (triangle->texture.type[1] == BUMP_MAP)
		n = get_bump_normal(&(triangle->texture.bump_map), &basis, n, \
			coord_3d_sub(p, triangle->c));
	else if (triangle->texture.type[1] == WAVE)
	{
		if (triangle->parent)
		{
			...
			c = coord_3d_add(triangle->p1, scalar_product(\
				coord_3d_sub(triangle->p2, triangle->p1), 0.5));
		basis.w = n;
