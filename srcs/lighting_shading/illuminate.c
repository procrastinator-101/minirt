...
t_rgb	illuminate(t_intersection hit, void **entities, t_ray ray)
{
	...
	p = coord_3d_add(ray.origin, scalar_product(ray.direction, hit.distance));
	n = surface_normal(hit.object, ray.direction, p, hit.type);
	l.origin = coord_3d_add(p, scalar_product(n, EPSILON));
	while (light)
	{
		if (light->brightness > 0.0)
		{
			l.direction = coord_3d_sub(light->position, l.origin);
